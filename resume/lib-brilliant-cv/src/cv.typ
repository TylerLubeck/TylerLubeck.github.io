...
#let _entry-styles(accent-color, before-entry-description-skip) = (
  ...
  // Same look as `b2` (gray oblique smallish), but without the right
  // alignment so the date can sit inline immediately after the title.
  date-inline: str => text(
    size: 8pt,
    weight: "medium",
    fill: gray,
    style: "oblique",
...
#let _make-cv-entry(
  ...
  if entry-type == "full" {
    ...
    table(
      ...
      table(
        ...
        table(
          ...
          row-gutter: 6pt,
          align: auto,
          // The date is rendered inline, right after the title, rather than
          // right-justified in its own column. The title is `society` when
          // society-first, otherwise `title`; the date follows whichever line
          // carries the actual job title.
          if society-first-setting {
            (styles.a1)(society)
          } else {
            {
              (styles.a1)(title)
      ...
      // Right column now carries only the location; the date moved inline
      // next to the title above. The empty cell keeps the location aligned
      // with the society line.
      table(
        columns: auto,
        inset: 0pt,
        stroke: 0pt,
        row-gutter: 6pt,
        align: auto,
        if society-first-setting { (styles.a2)(location) } else { [] },
        if society-first-setting { [] } else { (styles.b2)(location) },
      ),
    )
    ...
    if not multiple-dates {
      // Date inline, immediately to the right of the title, instead of
      // right-justified in a separate date column.
      {
        (styles.b1)(title)
        h(0.75em)
        (styles.date-inline)((styles.dates)(date))
